...
class FontPreference(context: Context) {
    ...
    var style by mutableStateOf(FontStyleKey.fromId(prefs.getString("style", FontStyleKey.Serif.id)))
        private set
...
enum class FontStyleKey(val id: String, val label: String, val desc: String) {
    ...
    companion object {
        fun fromId(id: String?): FontStyleKey = entries.firstOrNull { it.id == id } ?: Serif
    }
}
