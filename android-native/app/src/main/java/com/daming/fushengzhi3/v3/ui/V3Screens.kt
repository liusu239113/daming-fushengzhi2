...
private fun V3PeoplePage(state: V3GameState, controller: V3GameController) {
    ...
    V3Section("族人", "家族不是一开始就一堆人，而是慢慢生、慢慢养、慢慢分工")
    V3SelectorChips("族谱", people.map { it.id.toString() to "${it.name}${if (it.age < 12) "(${it.age})" else ""}" }, selectedPersonId.toString()) { selectedPersonId = it.toInt() }
...
private fun V3StrategyPage(state: V3GameState, controller: V3GameController) {
    ...
    V3Panel {
        ...
        V3SmallButton("写入家乘终局", Modifier.fillMaxWidth(), onClick = controller::finalizeGame)
    }
    V3Panel {
    ...
    V3Panel {
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
            site.taskTypes.take(3).forEach { task ->
                ...
                val label = if (person == null) task.label else "${person.name.takeLast(2)}${task.label}"
                V3SmallButton(label, Modifier.weight(1f), enabled = person != null) {
...
private fun V3PersonCard(person: V3Person, state: V3GameState, controller: V3GameController) {
    V3Panel {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.Top) {
            ...
            Text(if (person.currentTask == null) "待命" else "${person.currentTask.label}@${assignedSite?.name ?: "未知"}", color = if (person.currentTask == null) V3Muted else V3Green, fontSize = 13.sp, fontWeight = FontWeight.Bold)
        }
        ...
        if (person.age < 12) {
            Text("尚年幼，暂不能办事。长大后可读书、管田、行商、从军。", color = V3Ink, fontSize = 13.sp)
        } else {
    ...
    tasks.chunked(3).forEach { row ->
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
            row.forEach { task ->
                ...
                V3SmallButton(task.label, Modifier.weight(1f), enabled = site != null && person.currentTask == null) {
    ...
    Column(Modifier.fillMaxWidth().background(Color(0xEE1F1712)).padding(horizontal = 10.dp, vertical = 8.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(7.dp)) {
                V3SmallButton("玩法", Modifier.width(66.dp)) { controller.openPlayGuide() }
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(7.dp)) {
            ...
            V3Metric("人", V3GameEngine.alivePeople(state).size, V3Blue, Modifier.weight(1f))
            V3Metric("产", V3GameEngine.builtSiteCount(state), V3Red, Modifier.weight(1f))
        }
    }
}
...
private fun V3EventDialog(event: V3ActiveEvent, controller: V3GameController) {
    Dialog(onDismissRequest = {}) {
        V3Panel(Modifier.widthIn(max = 500.dp)) {
...
private fun V3Dialog(title: String, onDismiss: () -> Unit, content: @Composable ColumnScope.() -> Unit) {
    Dialog(onDismissRequest = onDismiss) {
        V3Panel(Modifier.widthIn(max = 460.dp)) {
...
private fun nextAdvice(state: V3GameState): String = when {
    ...
    V3GameEngine.alivePeople(state).none { it.age >= 12 && it.currentTask == null } -> "等待子嗣成长，维持产业月产。"
    else -> "选择产业派人经营，再推进月结。"
}
...
private fun bestPersonFor(people: List<V3Person>, task: V3TaskType): V3Person? {
    return people.filter { it.alive && it.age >= 12 && it.currentTask == null }.maxByOrNull { person ->
        when (task) {
